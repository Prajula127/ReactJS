...
class App extends Component {
  ...
  incrementCartItemQuantity = id => {
    this.setState(prevState => ({
      cartList: prevState.cartList.map(each => {
        if (each.id === id) {
          const incrementQuantity = each.quantity + 1
          return {...each, quantity: incrementQuantity}
        }
        return each
      }),
    }))
      ...
      cartList: prevState.cartList.map(each => {
        if (each.id === id) {
          const decrementQuantity =
            each.quantity > 1 ? each.quantity - 1 : this.removeCartItem(id)
          return {...each, quantity: decrementQuantity}
        }
        return null
      }),
    }))
