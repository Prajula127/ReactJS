...
class MatchGame extends Component {
  ...
  randomImages = () => {
    ...
    const imageListId = imagesList[index].id
    this.setState({imageListId, score: score + 1})
  }
        ...
        <ul className="tab-lists">
          {tabsList.map(each => (
            <TabItem
              key={each.tabId}
              tabsList={each}
              clickTab={this.clickedTabItem}
              isTabActive={each.tabId === activeTabId}
            />
          ))}
        </ul>
      </div>
        ...
        <ul className="game-lists">
          {filterImages.map(each => (
            <GameItem
              key={each.id}
              gameList={each}
              clickImage={this.clickImageItem}
            />
          ))}
        </ul>
      </div>
  ...
  getImage = () => {
    const {imageListId} = this.state
    const image = imagesList.find(each => each.id === imageListId)
    return image
  }
  ...
  render() {
    ...
    const {category, imageUrl} = image
    // console.log(image)
    return (
      <div className="app-store">
        <NavbarItems score={score} timer={timer} />
        <div className="bottom-container">
          {gameOver ? (
            <GameOver score={score} gameOver={this.clickGameOver} />
          ) : (
            <div className="bottom-card">
              <div className="img-container">
                <img className="img" alt={category} src={imageUrl} />
              </div>
              {this.renderTabItem()}
